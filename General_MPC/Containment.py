...
def Containment_Offline(party: TrustedDealer, knots_list: TriFSSTensor, filename=None, sec_para=fss_config.sec_para,
    ...
    for i in range(length - 1):
        ...
        key_0, key_1 = keygenCorrelatedDIF(interval=interval, party=party, sec_para=sec_para,
...
def Containment(party: SemiHonestParty, x: GroupElements,
                offline_pack_file: str, sec_para=fss_config.sec_para, DEBUG=fss_config.DEBUG) -> TriFSSTensor:
    """
    ...
    for i in range(offline_data.len):
        ...
        result_tensor.add_elements(index_result)

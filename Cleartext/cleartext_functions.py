...
def clear_sin(x: GroupElements) -> GroupElements:
    ...
    sin_var: LookUpTable = api.local_recv(f'publicSin_{repr_config.bitlen}_{repr_config.scalefactor}_None.lut')
    res = GroupElements(0)
...
def clear_cos(x: GroupElements) -> GroupElements:
    """
    This function is the cleartext implementation of cos, for accuracy evaluation only.
    :param x:
    :return:
    """
    x = lib.clear_bits_removal(x, (x.bitlen - 2 - x.scalefactor))
    x = lib.clear_mod(x, 2, unsigned=True)
    containment_result = lib.clear_containment(x, [0, 0.5, 1, 1.5, 2])
    coefficients: LookUpTable = api.local_recv(f'publicCosCoefficients_{repr_config.bitlen}_'
                                               f'{repr_config.scalefactor}.lut')
    sin_var: LookUpTable = api.local_recv(f'publicCos_{repr_config.bitlen}_{repr_config.scalefactor}_None.lut')
    res = GroupElements(0)
    # bit-length of coefficients should equal to the truncated x
    a = 0
    b = 0
...
def clear_tan(x: GroupElements) -> GroupElements:
    """
    This function is the cleartext implementation of tangent, for accuracy evaluation only.
    :param x:
    :return:
    """
    x = lib.clear_bits_removal(x, (x.bitlen - 1 - x.scalefactor))
    x = lib.clear_mod(x, 1, unsigned=True)
    containment_result = lib.clear_containment(x, [0, 0.5, 1])
    coefficients: LookUpTable = api.local_recv(f'publicTanCoefficients_{repr_config.bitlen}_'
                                               f'{repr_config.scalefactor}.lut')
    tan_var: LookUpTable = api.local_recv(f'publicTan_{repr_config.bitlen}_{repr_config.scalefactor}_None.lut')
    res = GroupElements(0)
    # bit-length of coefficients should equal to the truncated x
    a = 0
    b = 0
